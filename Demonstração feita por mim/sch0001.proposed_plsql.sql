...
-- synchronize;
   ...
   if p_sch_name_0000413 is NULL then
      p_msg_return := 'Name must be informed.';
      return FALSE;
   end if;

   if p_sch_address_0000514 is NULL then
      p_msg_return := 'Address must be informed.';
      return FALSE;
   end if;

   if p_sch_country_0000918 is NULL then
      p_msg_return := 'Country must be informed.';
      return FALSE;
   end if;

   if p_sch_city_0000615 is NULL then
      p_msg_return := 'City must be informed.';
      return FALSE;
   end if;

   if p_sch_country_0000918 = 1 and p_sch_state_0001118 is NULL then
      p_msg_return := 'State must be informed for country = USA.';
      return FALSE;
   end if;

   if p_sch_country_0000918 != 1 and p_sch_state_0001118 is not NULL then
      p_msg_return := 'State must not be informed for other countries.';
      return FALSE;
   end if;
END;
